...
- **STALE_WRITE**: gawt remembers your last read of each file. A write is rejected with `reason: "STALE_WRITE"` (re-read and retry) when either (a) another agent changed the file since your last read, or (b) you never read the file at all and it already exists — you must `read_file` before touching a file someone else owns. You never pass a SHA.
